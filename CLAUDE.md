...
(ablation #4 fixed-weight 1{AoI>τ} penalty, `--aoi_pen_type indicator --aoi_pen_w`).
...
| `hard_seedN_t8e10` | 2–11 | 300 ep, integral dual | headline hard arm; n=10 CI; limit-cycle source |
| `hard_seedN_t8e10_pid` | 2–11 | 300 ep, PID dual | headline PID arm; n=10 CI |
| `hard_seedN_t{8,10,12}e{10,15}(_pid)` | 2–7 | 300 ep | τ/ε phase diagram (integral + PID) |
| `hard_seedN_t8e10_anneal` | 2–7 | 300 ep, σ-anneal | stability ablation (σ-anneal — rejected) |
| `hard_seedN_t8e10(_pid)_floor` | 2,3,4 | 300 ep, +floor | feasibility safeguard (retired — back-fires under PID) |
...
Eight reports, each auto-generated by a detached driver then committed; all numbers were
cross-checked against raw `.mat`. (Chronological batch logs — early rows may report
numbers later superseded; trust the findings box above + the Manuscript canon.)

| file | batch / condition | what it establishes |
|---|---|---|
| `RQ1_REMOTE_REPORT.md` | headline + integral τ/ε phase + floor, 6 seeds, 300 ep | first soft-vs-hard story (integral-era; some numbers later superseded) |
| `RQ1_STABILITY_REPORT.md` | σ-anneal vs PID vs baseline | PID dominates integral; σ-anneal rejected |
| `RQ1_PHASE_PID_REPORT.md` | PID τ/ε phase diagram | frontier SOFTENED not overturned |
| `RQ1_FLOOR_AND_CI_REPORT.md` | PID+floor (n=3) and n=10 CI | floor back-fires under PID; soft-vs-PID gap holds; PID-#pass weakens (CI overlap) |
...
Several `results_remote/` figures are integral-era and RETIRED for the paper (see
`../Manuscript/README_FOR_WRITING.md`).
...
- **Claim 3** — soft vs PID (all 30 feasible) of violation / mean AoI (`AoI.mat[:,-100:].mean`)
  ...
  → `fig_claim3_cost.png`.
- **Claim 4** — **300-ep** `hard_*_t8e10` vs `_pid`: worst platoon's per-episode
  `viol_rate.mat` last-100-ep std ≈ 0.175 (integral) → 0.097 (PID), concentrated on s3,s7.
  → `fig_claim4_pid_stability.png`.
