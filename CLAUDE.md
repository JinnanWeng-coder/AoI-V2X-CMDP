...
Five live reports in `results_remote/` (table below), each auto-generated by a detached
...
Manuscript canon.) Three retired process reports moved to `model/Legacy_300ep/`, and the
scenario-sweep report is in `model/ScenarioSweep/` (see note under the table).
...
`fig_phase_diagram(_pid).png`, `fig_stability_*.png`, `fig_(pid_)floor.png`.
