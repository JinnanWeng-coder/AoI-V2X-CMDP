...
class Environ:
    ...
    def __init__(self, down_lane, up_lane, left_lane, right_lane, width, height, n_veh, size_platoon, n_RB,
        ...
        # [RQ1-CMDP] per-platoon AoI constraint configuration (overridden by Main.py).
        # constraint_mode == 'soft' : AoI enters the reward as a penalty (original).
        # constraint_mode == 'hard' : AoI is a per-platoon constraint P(AoI>tau)<=eps;
        #                             the reward penalty is disabled (aoi_penalty_coef=0).
        ...
        self.Interference_all = np.zeros(int(self.n_Veh / self.size_platoon)) + self.sig2_dB
    ...
    def add_new_platoon_by_number(self, number_vehicle, size_platoon):
        # due to the importance of initial positioning of platoons for RL, we have allocated
        # their positions as follows (to prevent the potential accidents) although it does not have
        # any operational impact in the network performance:
    ...
    def act_for_training(self, actions):
        ...
        per_user_task1_reward = np.zeros(int(self.n_Veh / self.size_platoon))
        per_user_task2_reward = np.zeros(int(self.n_Veh / self.size_platoon))
        ...
        cost_aoi = (np.asarray(platoon_AoI, dtype=np.float64) > self.tau_aoi).astype(np.float64)
        return per_user_task1_reward, per_user_task2_reward, global_reward, cost_aoi, platoon_AoI, C_rate, \
