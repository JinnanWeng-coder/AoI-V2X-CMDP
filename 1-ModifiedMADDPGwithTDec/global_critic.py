...
from Classes.G_network import G_CriticNetwork as CriticNetwork
...
class Global_Critic():
    ...
    def global_learn(self, agents_nets, state, action, reward_g, reward_t1, reward_t2, reward_cost, state_, terminal):
        ...
        rewards_t1 = T.tensor(reward_t1, dtype=T.float).to(self.global_critic1.device)
        rewards_t2 = T.tensor(reward_t2, dtype=T.float).to(self.global_critic1.device)
        rewards_cost = T.tensor(reward_cost, dtype=T.float).to(self.global_critic1.device)  # [RQ1-CMDP]
        ...
        target_actions = target_actions + T.clamp((T.randn_like(target_actions) * 0.2), -0.5, 0.5)
