...
| `Ablations_ep600/eps_sweep/` | A2 ε-sensitivity sweep (`--eps`, PID, τ=8; knob→level). ε=0.10 reused from Canonical | `hard_*_t8e5_pid_ep600` (ε=0.05), `hard_*_t8e20_pid_ep600` (ε=0.20) | 2–7 | `../../Canonical_ep600` PID (ε=0.10) | `results_remote/RQ1_ABLATION_EPS_SWEEP.md` | A2: worst-platoon tracks ε + λ relaxes (constraint, not penalty — positive counterpart to #4). Driver `a2_eps_sweep_driver.ps1` |
